    ...
    if (args.length <= 0) {
      ...
      switch (args[0]) {
        ...
        case "color":
          ...
          break;
        case "ticketlogs":
          eustaquio.setTicketLogsChannelId(textChannel.getId());
          command.reply("Successfully operation, all logs send to this channel.");
          Bot.getInstance().getEustaquioManager().getEustaquioObjectRepository().save(eustaquio);
          break;
        case "setuptickets":
          ...
          textChannel.sendMessageEmbeds(ticketsEmbed.build()).queue(message -> {
        ...
        case "clearmessages":
          if (args.length < 2) {
            ...
            command.getChannel().sendMessageEmbeds(usage.build()).queue();
          } else {
            if (NumberUtils.isParsable(args[1])) {
              ...
              for (Message message : messagesAmount) {
                command.getChannel().deleteMessageById(message.getId()).queue();
              }
